...
if( !class_exists( 'YITH_Delivery_Date_Order_Manager' ) ) {
    ...
    class YITH_Delivery_Date_Order_Manager
    {
        ...
        public function __construct()
        {
            ...
            add_action( 'save_post', array( $this, 'save_order_meta' ),99 );
        ...
        public function save_order_meta( $post_id ){
            ...
            if( 'shop_order' == $post_type && isset( $_POST['ywcdd_has_date'] ) && 'yes' == $_POST['ywcdd_has_date'] ){
                ...
                update_post_meta( $post_id , 'ywcdd_order_shipped', $shipped );
                $email_is_sent = get_post_meta( $post_id, '_ywcdd_email_sent', true );
               
                if( 'yes' === $shipped && empty( $email_is_sent ) ) {

                    WC()->mailer();
                    do_action( 'yith_advise_user_delivery_email_notification', $post_id );
                }
        ...
        public function custom_columns( $column_name, $order_id ){


            if( 'shipping_date' == $column_name ){

                $ship_date = get_post_meta( $order_id, 'ywcdd_order_shipping_date', true );
